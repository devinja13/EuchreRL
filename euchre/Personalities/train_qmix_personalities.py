"""
QMIX cooperative training for Euchre with personality-based repward shaping.

Usage:
    python train_qmix_personalities.py --personality aggressive
    python train_qmix_personalities.py --personality timid

Team 0&2 (QMIX): two agents trained cooperatively.
Team 1&3: fixed EuchreRuleAgents (opponents).

Personality reward shaping (on top of base game payoff):

  Aggressive — encourages risk-taking, calling trump, and going for marches:
    • Trick reward:      +0.35 per trick (higher than neutral 0.25)
    • Trump call bonus:  +0.3 one-time when the team calls trump
    • March bonus:       extra +0.5 on top of the +2 base payoff
    • Euchre penalty:    softened to 60% of base (-1.2 instead of -2)

  Timid — encourages conservative play, avoiding euchres:
    • Trick reward:      +0.15 per trick (lower than neutral 0.25)
    • Trump call penalty: -0.2 one-time when the team calls trump
    • Euchre penalty:     amplified by extra -1.0 (total -3)
    • Defense bonus:      +0.3 when team wins without having called trump
"""
...
def run_episode(env, qmix: QMIXSystem, opp_agents: dict,
                joint_memory: JointMemory,
                personality: str = 'neutral') -> float:
    """
    Play one full hand and store joint transitions with personality-shaped rewards.
    ...
    team_called_trump = False  # track whether our team called trump

    # ── personality-specific reward parameters ────────────────────────────────
    if personality == 'aggressive':
        trick_reward_scale = 0.35   # higher per-trick reward
        trump_call_bonus   = 0.3    # bonus for calling trump
        march_bonus        = 0.5    # extra reward for march (5-0 sweep)
        euchre_scale       = 0.6    # soften euchre penalty (encourage risk)
        defense_bonus      = 0.0    # no bonus for defensive wins
    elif personality == 'timid':
        trick_reward_scale = 0.15   # lower per-trick reward
        trump_call_bonus   = -0.2   # penalty for calling trump
        march_bonus        = 0.0    # no extra march bonus
        euchre_scale       = 1.5    # amplify euchre penalty (discourage risk)
        defense_bonus      = 0.3    # bonus for winning without calling
    else:
        trick_reward_scale = 0.25
        trump_call_bonus   = 0.0
        march_bonus        = 0.0
        euchre_scale       = 1.0
        defense_bonus      = 0.0
    ...
    while not env.is_over():
        ...
        # ── detect if our team is calling trump ───────────────────────────────
        if player_id in (0, 2) and not team_called_trump:
            action_str = ACTION_LIST[action]  # decode action_id → string
            if action_str == 'pick' or action_str.startswith('call'):
                team_called_trump = True
        ...
        # ── flush joint transition with shaped reward ─────────────────────────
        if hand_ended:
            ...
            if team_won and team_tricks == 5:
                # March — reward aggressive, neutral for timid
                shaped_reward += march_bonus
            elif not team_won and team_called_trump:
                # Euchred — soften for aggressive, amplify for timid
                shaped_reward = base_payoff * euchre_scale
            elif team_won and not team_called_trump:
                # Defensive win — reward timid
                shaped_reward += defense_bonus

            flush(shaped_reward, done=True)
        elif trick_ended:
            new_tricks  = game.score[0] + game.score[2]
            trick_r     = (new_tricks - prev_team_tricks) * trick_reward_scale
            prev_team_tricks = new_tricks
            flush(trick_r)
        elif 0 in buf and 2 in buf and len(game.center) == 0:
            # Both acted during bidding — give trump-call bonus/penalty
            reward = trump_call_bonus if team_called_trump else 0.0
            flush(reward)
...
if __name__ == '__main__':
    ...
    # ── training ──────────────────────────────────────────────────────────────
    print("=" * 60)
    print(f"QMIX Cooperative Euchre Training — {personality.upper()} personality")
    print("  Team QMIX : Player 0 + Player 2 (QMIX)")
    ...
    for ep in range(1, EPISODES + 1):
        run_episode(env, qmix, opp_agents, joint_memory, personality)
        ...
        if ep % EVAL_EVERY == 0:
            ...
            # save checkpoint so progress isn't lost if stopped early
            ckpt_dir = os.path.dirname(os.path.abspath(__file__))
            ckpt_path = os.path.join(ckpt_dir, f'qmix_{personality}.pt')
            qmix.save(ckpt_path)
    ...
    save_name = f'qmix_{personality}.pt'
    qmix.save(os.path.join(os.path.dirname(os.path.abspath(__file__)), save_name))
    ...
    ax1.set_title(f'QMIX ({personality}) vs Rule Agents — Learning Curves')
    ax1.legend()
    ...
    plt.tight_layout()
    plot_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'qmix_{personality}_learning_curve.png')
